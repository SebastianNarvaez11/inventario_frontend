...
import { fetchProviders, deleteProvider } from '../../redux/actions/providerActions'
...
import ModalCreateProvider from '../../components/modals/proveedores/ModalCreateProvider'
import ModalUpdateProvider from '../../components/modals/proveedores/ModalUpdateProvider'

const Providers = () => {

    const { providers, isFetchingProviders } = useSelector(state => state.providerReducer)
    ...
    const handleDeleteProvider = (row) => {
        ToastDelete(`¿ Esta seguro de eliminar el proveedor "${row.nombre}" de forma permanente?`)
            .fire().then((result) => {
                if (result.value) {
                    dispatch(deleteProvider(row))
                }
            })
    }
            ...
            render: (text, row) => {
                return (
                    <>
                        ...
                        <span className="delete-table" onClick={() => handleDeleteProvider(row)}>
                ...
                <div className='d-flex flex-column justify-content-center mt-9 animate__animated animate__fadeIn'>
                    <div className='text-center'>
                        <Loader
                            ...
                            width={100}
                        />
                        <h1 className=' mt-3'>Cargando Proveedores</h1>
                ...
                <div style={{ paddingLeft: 50 }}>
                    ...
                    <Row style={{ paddingRight: 90, marginTop: 50 }}>
                        <Button className='btn mb-4' style={{ backgroundColor: '#14cc60', border: 0, width: 'auto' }} type="button" onClick={toggleCreate}>
                            <i className="fas fa-plus mr-2"></i> Añadir Proveedor
                        </Button>
                    ...
                    <ModalCreateProvider show={showCreate} toggle={toggleCreate} />
                    {showUpdate.data && <ModalUpdateProvider show={showUpdate.show} data={showUpdate.data} toggle={toggleCloseUpdate} />}
                </div>
            }
        </>
    )
