...
const ModalUpdateProduct = ({ show, data, toggle }) => {

    const { isUpdatingProduct } = useSelector(state => state.productReducer)
    ...
    const [itemsFabricacion, setItemsFabricacion] = useState(data.items)
    ...
    const upItemFabricacion = (item_up) => {
        setItemsFabricacion(itemsFabricacion.map(item => item.material.id == item_up.material.id ? (item = item_up) : item))
    }

    const delItemFabricacion = (item_del) => {
        setItemsFabricacion(itemsFabricacion.filter(item => item.material.id !== item_del.material.id))
    }
        ...
        <Modal isOpen={show} className='modal-xl'>
            <Formik
                ...
                onSubmit={(values, formikBag) => {
                    ...
                    console.log(request)
                    dispatch(updateProduct(request, closeAndClean))
                ...
                return (
                    <Form>
                        <ModalHeader style={{ backgroundColor: '#ffb703' }} className='d-flex justify-content-center'>
                            <span style={{ fontSize: '20px', color: 'white' }}>Modificar Producto</span>
                        </ModalHeader>
                                            ...
                                            return {
                                                onClick: event => {
                                                    if (record.existencia <= 0) {
                                                        ...
                                                        const items = itemsFabricacion.filter(item => item.material.id == record.id)
                                                        if (items.length !== 0) {
                                ...
                                <Col lg='6' style={{ paddingLeft: 90 }}>
                                    ...
                                    {itemsFabricacion.length == 0
                                        ...
                                        <div className='mt-3' style={{ height: 450, overflowY: 'scroll', padding: 30 }}>
                                            {itemsFabricacion.map(item => (
                                                <Row className='items-compra d-flex align-items-center animate__animated animate__pulse mb-4' key={item.material.id}>
                        ...
                        <ModalFooter className='d-flex justify-content-end'>
                            <div>
                                <Button style={{ backgroundColor: '#ffb703', borderColor: '#ffb703', marginRight: 10 }} disabled={isSubmitting || !isValid || isUpdatingProduct || itemsFabricacion.length == 0} type='submit'>Modificar</Button>
